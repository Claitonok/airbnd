  ...
  return (
    <div className='flex items-center gap-4 overflow-x-auto'>
       {/* Navegação de filtros */}
        ...
        {iconsAirbnb.map((icon, indice) => (
            <SwiperSlide key={indice}>
                <Link href={icon.url} className='flex flex-col items-center gap-2'>
                <Image
                    src={icon.source}
                    ...
                    /> */}
                    <span className='text-sm text-center whitespace-normal'>{icon.description}</span>
                </Link>
            </SwiperSlide>
        ))}
        </Swiper>
